...
import { maskPaidCalloutBlocks, flattenAllCallouts } from '@/lib/paidContent'
...
export function preferStablePdfSignedUrls(recordMap) {
  Object.values(recordMap?.block || {}).forEach(entry => {
    ...
    if (
      !block?.type ||
      !['pdf', 'file', 'audio', 'video'].includes(block.type) ||
      !source ||
    ...
    // attachment: / S3 等需走 Notion signed，保证博客访客可点开下载
    if (
      !(
 ...
 * @returns
 */
export function formatNotionBlock(block, options = {}) {
  const maskPaid = options.maskPaid !== false
  ...
  for (let i = 0; i < blocksToProcess.length;) {
    ...
    if (
      ...
      // 已被误替换的占位图不要再包一层 signed
      if (oldUrl?.includes('placeholder.com')) {
        i++
        continue
      }
  ...
  // 自定义 Callout 不渲染 children：先把子块文字并进 title
  flattenAllCallouts(clonedBlock)

  // 公开渲染前剥离【付费】/【隐藏】Callout 正文，避免进 HTML / 源码
  if (!maskPaid) {
    return clonedBlock
  }
  const { block: masked } = maskPaidCalloutBlocks(clonedBlock)
  return masked
}
...
function sanitizeBlockUrls(blockValue) {
  ...
  const fixUrl = (url) => {
    ...
    // Notion 附件协议：file/pdf/image/embed 等后续会转 signed URL，绝不能改成占位图
