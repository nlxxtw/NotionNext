...
export default async function getPageProperties(
  ...
  for (let i = 0; i < rawProperties.length; i++) {
    ...
    if (schema[key]?.type && !excludeProperties.includes(schema[key].type)) {
      ...
      switch (schema[key]?.type) {
        ...
        case 'checkbox': {
          // Notion unofficial API: checked ≈ [["Yes"]]
          properties[schema[key].name] = toStickyBool(getTextContent(val))
          break
        }
        case 'select':
        case 'multi_select': {
  ...
  // sticky / 置顶 Checkbox → boolean
  properties.sticky = toStickyBool(properties.sticky ?? properties['置顶'])
