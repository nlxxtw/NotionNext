...
async function getPageAISummary(props, pageContentText) {
  const notionCfg = props?.NOTION_CONFIG || {}
  const aiSummaryAPI = siteConfig('AI_SUMMARY_API', '', notionCfg)
  const aiSummaryKey =
    siteConfig('AI_SUMMARY_KEY', '', notionCfg) ||
    siteConfig('TianliGPT_KEY', '', notionCfg)
  if (!aiSummaryAPI || !aiSummaryKey || !props?.post) return
  ...
  const wordLimit = siteConfig('AI_SUMMARY_WORD_LIMIT', '1000', notionCfg)
  let content = ''
