...
/**
 * 解析用于 IndexNow 的站点 URL（避免仍用默认 tangly1024.com）
 */
export function resolveIndexNowSiteUrl(notionConfig) {
  const configured = String(
    siteConfig('LINK', '', notionConfig) || BLOG.LINK || ''
  ).trim()
  if (configured && !/tangly1024\.com/i.test(configured)) {
  ...
  if (fromEnv) {
    const withProto = /^https?:\/\//i.test(fromEnv)
      ? fromEnv
      : `https://${fromEnv}`
    return normalizeSiteUrl(withProto)
  }

  return normalizeSiteUrl(configured || BLOG.LINK)
}
...
export function resolveIndexNowKey(notionConfig) {
  ...
  try {
    const host = new URL(resolveIndexNowSiteUrl(notionConfig)).hostname.replace(
      /[^a-zA-Z0-9_-]/g,
      ''
    )
...
export function getIndexNowKeyLocation(siteUrl, notionConfig) {
  ...
  // IndexNow 规范：根目录 /{key}.txt（内容即密钥本身）
  const key = resolveIndexNowKey(notionConfig)
  return createSiteUrl(siteUrl, `${key}.txt`)
}
...
export async function submitIndexNowUrls({
  ...
  const siteUrl = resolveIndexNowSiteUrl(notionConfig)
  const resolvedHost =
    host ||
    (() => {
      try {
